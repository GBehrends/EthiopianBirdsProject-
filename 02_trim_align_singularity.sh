...
#SBATCH --array=$(wc -l <species>/00fastq/SampleID.txt)

# Script be submitted when logged into quanah directly, will not work if submitted from login node. 
# Many programs will not overwrite previous files generated by running this script from before. DELETE THEM ALL FIRST.
