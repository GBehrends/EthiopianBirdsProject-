...
proj_dir <- "<location of main working directory"
setwd(paste0(proj_dir, "species/msmc_vcf")) # Setting wd to location of vcf files
sam_loc <- paste0(proj_dir, "species/00fastq/SampleID.txt") # Location of Sample ID list 
out_dir <- paste0(proj_dir, "MSMC2/species_demography") # Location to put msmc files

# set up names of individuals
ind_names <- scan(sam_loc, character(), quote = "")

# column headers
vcf_colnames <- c("POS", "REF", "ALT", seq(from = 1, to = length(ind_names)))
options(scipen=999)


# all the files to read
x_files <- list.files(getwd(), pattern="*.simple.vcf")
...
x_unique <- unique(sapply(strsplit(x_files, "\\."), "[[", 1))
...
for(a in 1:length(x_unique)) {
	...
	b_files <- x_files[sapply(strsplit(x_files, "\\."), "[[", 1) %in% x_unique[a]]
