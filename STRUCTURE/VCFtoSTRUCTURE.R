...
outputfile <- "<location of output file>"
...
for(a in 1:length(x_unique)) {
  ...
  if(a == 1) {
    write(name_header, file=, sep="\t", ncolumns=50)
  }
  ...
  # write to output
  write.table(output2, file=outputfile, sep="\t", quote=F, row.names=F, col.names=F, append=T)
  
}

# rewrite the output transposed
x <- read.table(file=outputfile, sep="\t", stringsAsFactors=F)
x <- t(x)
write.table(x, file=outputfile, sep="\t", quote=F, row.names=F, col.names=F)
